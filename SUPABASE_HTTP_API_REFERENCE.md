# PocketBase Raw HTTP API Reference

Scraped from official PocketBase and PostgREST documentation. This covers the raw HTTP layer
...
| Service       | URL Pattern                                                        |
|---------------|--------------------------------------------------------------------|
| REST API      | `https://<PROJECT_REF>.pocketbase.co/rest/v1/`                       |
| Auth          | `https://<PROJECT_REF>.pocketbase.co/auth/v1/`                       |
| Storage       | `https://<PROJECT_REF>.pocketbase.co/storage/v1/`                    |
| Realtime WS   | `wss://<PROJECT_REF>.pocketbase.co/realtime/v1/websocket`            |
| Realtime REST | `https://<PROJECT_REF>.pocketbase.co/realtime/v1/api/broadcast`      |
| Edge Functions| `https://<PROJECT_REF>.pocketbase.co/functions/v1/<function-name>`   |
| JWKS          | `https://<PROJECT_REF>.pocketbase.co/auth/v1/.well-known/jwks.json` |
| TUS Upload    | `https://<PROJECT_REF>.storage.pocketbase.co/storage/v1/upload/resumable` |
...
- PocketBase-issued: `GET /auth/v1/user` (returns 200 if valid)
...
```
wss://<PROJECT_REF>.pocketbase.co/realtime/v1/websocket?apikey=<ANON_KEY>&vsn=1.0.0
```
...
#### Setup requirements
1. Add table to publication: `ALTER PUBLICATION pocketbase_realtime ADD TABLE <table>`
...
#### Send via REST API
```
POST https://<PROJECT_REF>.pocketbase.co/realtime/v1/api/broadcast
Headers:
  apikey: <KEY>
  Content-Type: application/json
Body: {
...
**Endpoint:** `https://<PROJECT_REF>.storage.pocketbase.co/storage/v1/upload/resumable`
...
### Invoke
```
POST https://<PROJECT_REF>.pocketbase.co/functions/v1/<function-name>
Headers:
  apikey: <KEY>
  Authorization: Bearer <TOKEN>
  Content-Type: application/json
...
PocketBase applies rate limiting at the API gateway level. Specific limits depend on the project's
...
```
postgresql://postgres.[PROJECT]:[PASSWORD]@aws-0-[REGION].pooler.pocketbase.com:6543/postgres
```
...
```bash
# From remote project
npx pocketbase gen types typescript --project-id "$PROJECT_REF" --schema public > database.types.ts

# From local dev
npx pocketbase gen types typescript --local > database.types.ts

# From connection string
npx pocketbase gen types typescript --db-url postgres://... --schema public > database.types.ts
```
